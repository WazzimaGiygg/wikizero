...
function endGame(message) {
    ...
    // Adicionado: Redesenha o mapa e os elementos uma última vez para que não fique tela preta
    ctx.clearRect(0, 0, canvas.width, canvas.height); // Limpa para desenhar novamente
    drawMap();
    drawPlayer();
    drawGhosts();
...
function gameLoop() {
    if (!isGameRunning) {
        // Se o jogo não estiver rodando, mas o loop foi chamado (o que não deveria acontecer com clearInterval),
        // desenhamos uma vez para garantir que o mapa esteja visível quando o HUD aparece.
        // Isso é mais uma garantia do que uma necessidade se o clearInterval funcionar perfeitamente.
...
function resetGame() {
    ...
    // Certifica-se de que o estado visual esteja pronto para um novo jogo
    ctx.clearRect(0, 0, canvas.width, canvas.height);
    drawMap();
    drawPlayer();
    drawGhosts();
